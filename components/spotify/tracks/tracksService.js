...
async function whom(response_url) {
    try {
        ...
        if(!onPlaylist(current_track.body.context, playlist_id)){
            await slack_controller.inChannelReply(`:information_source: Spotify is not playing from the playlist. Current Song: ${current_track.body.item.artists[0].name} - ${current_track.body.item.name}${current_track.body.item.explicit ? " (Explicit)" : ""}`, null, response_url);
            return;
            ...
            for (let offset = num_of_searches-1; offset >=0 ; offset--){
                ...
                if (index != -1){
                    ...
                    if (previous_track == null || found_track.added_by.id != spotify_user_id) {
                        ...
                        await slack_controller.inChannelReply(`:white_frowning_face: ${current_track.body.item.artists[0].name} - ${current_track.body.item.name}${current_track.body.item.explicit ? " (Explicit)" : ""} was added ${moment(found_track.added_at).fromNow()} directly to the playlist in Spotify by <${user_profile.body.external_urls.spotify}|${user_profile.body.display_name}>.`, null, response_url);
                        return;            
                    }
                    else{
                        await slack_controller.inChannelReply(`:microphone: ${current_track.body.item.artists[0].name} - ${current_track.body.item.name}${current_track.body.item.explicit ? " (Explicit)" : ""} was last added ${moment(previous_track.time_added).fromNow()} by <@${previous_track.user_id}>.`, null, response_url);
                        return;
                    }
                }
            }
