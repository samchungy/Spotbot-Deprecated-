    ...
    async findArtist(query, trigger_id, response_url) {
        try {
            ...
            if (search_artists.length == 0) {
                //No Artists found
                await this.slack_controller.reply(`:slightly_frowning_face: No artists found for the search term "${query}". Try another search?`, null, response_url);
                return;
            } else {
                // Store in our db
            ...
            await this.slack_controller.reply(`:slightly_frowning_face: Find artists failed. `, null, response_url);
        }
    }
    
    async getThreeArtists(trigger_id, page, response_url) {
        try {
            ...
            if (_.get(search, 'artists.length') == 0) {
                ...
                await this.slack_controller.reply(`:information_source: No more artists. Try another search.`, null, response_url);
                return;
            }
            ...
            slack_attachments.push(
                new this.slack_formatter.doubleButtonAttachment(`Page: ${page}/${search.total_pages}`, "See more artists", trigger_id, "See more artists", 
